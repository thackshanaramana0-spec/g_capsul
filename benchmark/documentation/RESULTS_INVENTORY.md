...
`results/` at the repo root holds **fourteen** run directories. Exactly **one** is the
...
| `claim2` | 7 loose CSVs from Claim 2 development — **read this row before citing any of them** | Several are named so they look authoritative: `method_b_full_chr20.csv`, `t3_t5_full_chr20_HG002.csv`, `t3_t5_full_chr20_headtohead.csv`. They are development output from the Method B work, predate the 2026-09-09 sweep, and use window/scoring conventions that were later changed. The published Claim 2 tables are `benchmark/results/claim2/claim2_T2.*.csv` and nothing else. |
| `phase_a` | the Phase A optimisation gate record, 16 files | Not a results table at all — it is the *evidence for decisions*, including reverted ones (`04_gate_A2_REVERTED.txt`) and a correction (`15_CORRECTION_incomplete_archive.txt`). **`allphases_14dataset.csv` here is VOID** — those are the pre-fix Phase 2b numbers for the 14-dataset set, measured before four silent data-loss bugs were found; see CLAUDE.md's warning on `docs/PHASE2B_RESULT.md`. |
