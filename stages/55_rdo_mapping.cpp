...
int main(int argc,char** argv){
    ...
    size_t n_matched=0;
    {
        ...
        const uint32_t MAXDIV=(getenv("MAXMAP_DIV")?atoi(getenv("MAXMAP_DIV")):1);
        const uint32_t MAXMAP=(uint32_t)(Lmax/MAXDIV);   // kept as a hard ceiling; RDO below is the real gate
        // STAGE 55. Replaces the fixed mismatch-COUNT threshold with a real
        // per-candidate COST comparison, in the spirit of RDO mode decision
        // (H.264/HEVC: J = distortion + lambda*rate, decided per block using
        // a CHEAP rate estimate from the current probability table -- "Fast
        // entropy-based CABAC rate estimation for mode decision", a table
        // lookup indexed by probability state, not a full re-encode). Ours:
        // sum of -log2(p) per mismatch, using REAL measured per-reference-
        // base rates (not one flat average), against a static estimate of
        // what that read would cost assembled instead.
        //
        // Honest limitation, not hidden: the "cost if assembled" side is NOT
        // locally computable per read -- it emerges from the WHOLE survivor
        // set's mutual overlap, known only after mapping finishes. A single
        // static average (measured from a reference run) stands in for it.
        // That is the one approximation in this design; the mapped-cost side
        // is real, measured, per-candidate data.
        //
        // Rates in bits*256 fixed point (avoids floats in the hot loop):
        // A=1.5767 C=1.4962 G=1.5017 T=1.5789, measured on this exact data
        // (mm_ref.bin/mm_obs.bin, stage 53's dump). b2() order is A,C,G,T.
        static const uint32_t MMRATE256[4]={404,383,384,404};
        const uint32_t ASSEMBLED_BUDGET_BITS256 =
            (getenv("ASM_BITS")?(uint32_t)atoi(getenv("ASM_BITS")):(90*8*256));
        // default: 90 B/read average survivor cost (measured, stage 54 DIV=3:
        // 1,097,084 B / 12,065 reads = 90.9 B/read after MEM-second removal)
                ...
                th.emplace_back([&,t,lo,hi]{
                    ...
                    for(size_t p=from;p<hi;++p){
                        ...
                        for(uint32_t q=ix;q<ment.size()&&MKEY(q)==(uint32_t)k;++q){
                            ...
                            const uint32_t lim=MAXMAP;               // hard ceiling only
                            // RDO budget: if this read is already placed
                            // (cur!=255), it must beat that placement's own
                            // cost, not just be "better by one mismatch" --
                            // same principle, expressed in bits instead of
                            // count. We do not know the PRIOR placement's
                            // exact cost without recomputing it, so cur (a
                            // count) is converted with the flat average rate
                            // as a conservative stand-in; the FIRST placement
                            // of a read is judged against the real budget.
                            const uint32_t budget256 = (cur==255)
                                ? ASSEMBLED_BUDGET_BITS256
                                : (uint32_t)cur*394u;      // 394 = flat avg (1.5446*256)
                            const uint32_t RL=rlen[rid];
                            if(st+RL>text.size()) continue;
                            // Same packed-word XOR popcount as before, but now
                            // ALSO bit-scanning the differing pairs within
                            // each word to identify which reference base each
                            // mismatch sits on, so its REAL per-base rate can
                            // be charged instead of a flat count. Early exit
                            // once cost exceeds budget (mirrors the old
                            // mm>lim break, same purpose).
                            uint32_t mm=0; uint64_t cost256=0;
                            const uint64_t rbase=woff[rid]*32ULL;
                            for(uint32_t j=0;j<RL;j+=32){
                                const uint32_t take=(RL-j<32)?(RL-j):32;
                                const uint64_t tword=tw32((uint64_t)st+j);
                                uint64_t d=w32(rbase+j)^tword;
                                if(take<32) d&=~0ULL<<(2*(32-take));
                                // pairs folds each differing 2-bit field down to its
                                // OWN low bit position -- same position it already
                                // has in `tword`, since the fold is d|(d>>1), not a
                                // renumbering. So ctzll(pairs) directly indexes into
                                // tword with no basepos/offset conversion needed.
                                uint64_t scan=(d|(d>>1))&0x5555555555555555ULL;
                                while(scan){
                                    const int bitpos=__builtin_ctzll(scan);
                                    const int refc=(int)((tword>>bitpos)&3ULL);
                                    cost256 += MMRATE256[refc];
                                    ++mm;
                                    scan &= scan-1;
                                }
                                if(cost256>budget256){ mm=lim+1; break; }  // sentinel: over budget
                            }
                            if(mm<=lim && cost256<=budget256){ hit[t].push_back({rid,(uint32_t)st}); hmm[t].push_back(mm); }
                        }
                    }
                });
            }
