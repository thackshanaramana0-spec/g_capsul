...
int main(int argc,char** argv){
            ...
            for(unsigned t=0;t<T;++t){
                ...
                th.emplace_back([&,t,lo,hi]{
                    for(size_t i=lo;i<hi;++i){
                        ...
                        const std::string& A=reads[a];
                        const uint32_t off=(uint32_t)A.size()-L;
    ...
    size_t n_matched=0;
    {
        ...
        std::vector<uint64_t> mkey; std::vector<uint32_t> mrid; std::vector<uint8_t> mpart;
        {
            ...
            for(uint32_t rid:leftovers){
                ...
                if(R.size()<SEEDW) continue;
                for(uint32_t p=0;p<NPARTS;++p){
                    const uint32_t off=p*SEEDSTRIDE;
                    if(off+SEEDW>R.size()) break;
                    uint64_t k; if(packW(R.data()+off,k)) tmp.push_back({k,rid,(uint8_t)p});
                }
            }
        ...
        auto scan=[&](const std::string& text){
            ...
            std::vector<std::vector<uint32_t>> hit(T);
            std::vector<std::thread> th; th.reserve(T);
                ...
                th.emplace_back([&,t,lo,hi]{
                    ...
                    for(size_t p=from;p<hi;++p){
                        ...
                        for(uint32_t q=ix;q<mkey.size()&&mkey[q]==k;++q){
                            ...
                            for(size_t j=0;j<R.size();++j) if(t2[j]!=R[j]){ if(++mm>MAXMAP) break; }
                            if(mm<=MAXMAP) hit[t].push_back(rid);
                        }
                    }
                });
            }
            for(auto& x:th) x.join();
            for(auto& v:hit) for(uint32_t rid:v)
                if(!matched[rid]){ matched[rid]=1; ++n_matched; }
        };
        scan(pg);
    ...
    const size_t main_pg_end = pg.size();
    {
        ...
        for(uint32_t i=0;i<n;++i){
            if(!admit[i]||reads[i].size()<SW) continue;
            uint64_t k; if(packSW(reads[i].data(),k)) pref[k].push_back(i);
        }
        sweep();
    ...
    { std::vector<std::string>().swap(reads); }
    { std::unordered_map<uint64_t,std::vector<uint32_t>>().swap(pref); }
