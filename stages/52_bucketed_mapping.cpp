// STAGE 52 -- bucketed (cache-blocked) mapping probes. Tests the hypothesis
// that mapping's random-order lookups into a past-L3 table (stage 26's own
// diagnosis) can be sped up by grouping probes by target table region before
// doing the lookup, instead of processing them in scan order.
//
// RESULT: real, but modest, and not worth adopting as the default.
//
// An isolated synthetic benchmark (64 MB table, matching the "past L3"
// framing) showed a dramatic win: 0.398s naive random-order vs 0.111s
// radix-bucketed (256 buckets, O(n) counting pass, not a full sort -- a full
// sort cost 2.588s and was a net LOSS). On the REAL data the win shrank to
// ~9% (0.64s -> 0.58s pigeonhole mapping): the real index is only ~14 MB
// (1.85M entries, packed), much smaller than the synthetic test's 64 MB and
// mostly L3-resident already on this hardware, so there is far less locality
// to recover. MEM matching's index (~17 MB) is similarly sized, so the same
// technique there would plausibly show the same modest pattern, not a
// breakthrough -- not implemented, given this result.
//
// Verified correct: PG_LITERAL, mapped count, and the full mismatch histogram
// are IDENTICAL to the unmodified mapping stage; literal.txt (the archive's
// dominant stream) is byte-identical. The raw per-read mismatch SYMBOL stream
// differs in a small number of places where a read has two equally-good
// placements -- reordering probes changes which tied candidate is found
// first. This is not a new nondeterminism: the original merge already races
// on `readMM[rid]` across threads and documents itself as tolerant of a
// stale read ("never a wrong answer"), so which of several EQUALLY GOOD
// placements wins a tie was already not guaranteed reproducible. The
// GUARANTEE the algorithm actually makes -- fewest mismatches found per read
// -- is unaffected, confirmed by the identical histogram and literal.
//
// Costs real RAM: 187 MB -> 317 MB (+130 MB) for the per-thread candidate
// and bucket vectors. That is a bad trade by this project's own standard --
// stage 43's comparable flat-index change cost 15 MB for a much larger,
// clearly justified win. Kept here as a documented, tested, available
// option, NOT folded into the default pipeline.
//
//   g++ -O3 -march=native -fopenmp -o bucketed_mapping 52_bucketed_mapping.cpp
...
int main(int argc,char** argv){
        ...
        auto scan=[&](const std::string& text){
            ...
            for(unsigned t=0;t<T;++t){
                ...
                th.emplace_back([&,t,lo,hi]{
                    // STAGE 52. Two passes instead of one, confined to this
                    // thread's own slice. `mfind` is an open-addressing probe
                    // into a table past L3 (stage 26's own diagnosis), and
                    // random-order probing into a table that size is latency-
                    // bound: an isolated benchmark on this exact machine showed
                    // 0.398s naive random-order vs 0.111s after grouping probes
                    // by target table region (radix bucket, O(n) -- NOT a full
                    // sort, which cost more than it saved, 2.588s, in the same
                    // test). The merge below already tolerates any processing
                    // order -- "a stale read costs a redundant verify, never a
                    // wrong answer" -- so reordering probes changes nothing
                    // about correctness, only when each lookup happens.
                    //
                    // Pass 1: roll the seed and apply the cheap filters (K2
                    // stride, the 2MB Bloom filter) exactly as before, but
                    // defer mfind -- just record (seedStart, key).
                    std::vector<std::pair<uint32_t,uint64_t>> cand;
                    {
                        ...
                        for(size_t p=from;p<hi;++p){
                            ...
                            if(seedStart<lo) continue;
                            if(K2>1 && (seedStart%K2)) continue;
                            if(!mmaybe(k)) continue;
                            cand.push_back({(uint32_t)seedStart,k});
                        }
                    }
                    // Pass 2: radix-bucket by the SAME hash+mask mfind uses, so
                    // candidates whose lookup lands in the same region of mtab
                    // are grouped together. 256 buckets matched the isolated
                    // benchmark's sweet spot (finer buckets pay more in the
                    // counting pass than they save in locality).
                    const unsigned RBITS=8; const unsigned RB=1u<<RBITS;
                    // msize (table size) is a power of 2, MMASK = msize-1, so
                    // log2(msize) is the position of MMASK's highest set bit + 1.
                    // Shifting the hash down by (log2(msize) - RBITS) keeps its
                    // TOP RBITS bits, i.e. which coarse region of the table it
                    // lands in -- same table, same mask, just a coarser view.
                    const unsigned MBITS = MMASK ? (64-__builtin_clzll(MMASK)) : 0;
                    const unsigned RSHIFT = (MBITS>RBITS) ? (MBITS-RBITS) : 0;
                    std::vector<uint32_t> rcnt(RB+1,0);
                    auto rbucket=[&](uint64_t k)->uint32_t{
                        const uint64_t h=mmix(k)&MMASK;
                        return (uint32_t)std::min<uint64_t>(h>>RSHIFT, RB-1);
                    };
                    for(auto& c:cand) ++rcnt[rbucket(c.second)+1];
                    for(unsigned i=0;i<RB;++i) rcnt[i+1]+=rcnt[i];
                    std::vector<std::pair<uint32_t,uint64_t>> bucketed(cand.size());
                    { std::vector<uint32_t> fill(rcnt.begin(),rcnt.end());
                      for(auto& c:cand) bucketed[fill[rbucket(c.second)]++]=c; }

                    // Pass 3: same lookup+verify logic as before, just walked
                    // in bucketed order.
                    for(auto& c : bucketed){
                        const size_t seedStart=c.first; const uint64_t k=c.second;
    ...
    // ── STAGE 47: the mismatch SYMBOL stream, built for real ────────────────
    // Every number this progression has reported for mismatches is an
    // ESTIMATE: readMM[i] counts, priced at PgRC2's own observed rate of 1.55
    // bits/mismatch. That is not a coder. PgRC2 itself gets there with two
    // real ideas (SeparatedPseudoGenomePersistence.cpp:930-944):
    //   - EXCLUSIVE coding: the decoder already knows the reference base at a
    //     mismatch position, so the symbol is one of 3, not 4
    //     (reorderingSymbolsExclusiveMismatchEncoding)
    //   - PPMd order-5 over the resulting symbol SEQUENCE, which is whatever
    //     the mismatch stream's own byte-order structure gives for free
    // Neither of those uses genomic context. The literature is specific about
    // what is being left on the table: Illumina substitution rates vary by
    // 10-11x depending on which of the 12 possible substitutions it is (A>C
    // transversions frequent, C>G rare), and by up to 100x depending on the
    // flanking bases (Stoler & Nekrutenko / Meacham et al., context-dependent
    // Illumina error analysis). A context model keyed on the reference base
    // (and ideally its neighbours) should beat both the naive 2-bit encoding
    // AND PgRC2's context-free exclusive+PPMd approach.
    //
    // This does not code anything yet -- it extracts the real (ref, observed)
    // pairs from the finished mapping, in read order, so several coder designs
    // can be measured honestly against the same ground truth rather than
    // against an estimate.
    if(getenv("DUMP_MM")){
