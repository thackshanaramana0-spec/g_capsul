...
int main(int argc, char** argv) {
    if (argc < 2) { fprintf(stderr, "usage: 38_merge_sweep <in.fq> [minov]\n"); return 1; }
    ...
    std::vector<uint32_t> byPrefix(n);
    for (uint32_t i = 0; i < n; ++i) byPrefix[i] = i;
    ...
    std::vector<uint32_t> nxt(n, NONE), prv(n, NONE), ovl(n, 0), head(n), tail(n);
    for (uint32_t i = 0; i < n; ++i) { head[i] = i; tail[i] = i; }
    ...
    for (uint32_t ov = L - 1; ov >= MINOV; --ov) {
        ...
        bySuffix.clear();
        for (uint32_t i = 0; i < n; ++i) if (nxt[i] == NONE) bySuffix.push_back(i);
        ...
        while (ia < bySuffix.size() && ib < byPrefix.size()) {
            ...
            for (size_t j = ib; j < byPrefix.size(); ++j) {
                ...
                if (A == B || nxt[A] != NONE || prv[B] != NONE || head[A] == B) continue;
    ...
    for (uint32_t i = 0; i < n; ++i) {
        ...
        pg_len += L;
        uint32_t cur = i;
        while (nxt[cur] != NONE) { pg_len += L - ovl[cur]; cur = nxt[cur]; }
    }
    lap("emit");
