...
int main(int argc,char** argv){
    ...
    // STAGE 99 -- real, measured (not organism-labeled) fix. Five bucket-
    // shrinking attempts (stages 93/95/96/97/98) all failed or regressed on
    // well-behaved genomes, because none of them addressed the actual cause:
    // real, inter-read repeat-family redundancy (confirmed directly -- the
    // most duplicated 32-mers in T. cacao are the plant telomeric repeat
    // motif TTTAGGG/AACCCT; general per-read complexity is NOT the signal,
    // measured directly: T. cacao's avg per-read 4-mer diversity is actually
    // HIGHER than C. elegans's, which never blew up). A handful of reads
    // belong to genuinely huge repeat families and will essentially never
    // yield a valid link (round2's overall hit rate on T. cacao was 0.01%);
    // every level's sweep still pays to walk their enormous buckets anyway.
    //
    // Fix: exclude such reads from `pent` ENTIRELY (not just from being
    // queries -- an earlier draft of this only skipped them as queries and
    // left them fully indexed as candidates, which does nothing, since other
    // normal reads' probes would still walk into the same huge bucket).
    // Threshold is a PERCENTILE of THIS FILE's own bucket-size distribution,
    // computed fresh per dataset from real data -- not a hardcoded number or
    // an organism/kingdom label (kingdom is a weak, non-causal proxy,
    // confirmed directly above). Bootstrapped in two passes: build once
    // un-filtered to measure the real distribution, then rebuild filtered.
    std::vector<uint32_t> bucketSizeOf;
    uint32_t repeatExcludeThresh=UINT32_MAX;   // UINT32_MAX = no exclusion (first pass)
    auto buildPref=[&](bool useAdmit){
        pent.clear(); pent.reserve(n);
        for(uint32_t i=0;i<n;++i){
            if(useAdmit && !admit[i]) continue;
            if(rlen[i]<SW) continue;
            if(!bucketSizeOf.empty() && bucketSizeOf[i]>=repeatExcludeThresh) continue;
    ...
    buildPref(false);
    lap("prefix seed index (pass 1, unfiltered)");

    // Measure this file's own bucket-size distribution from the unfiltered
    // pass, set a real threshold, then rebuild pent excluding the tail.
    {
        bucketSizeOf.assign(n,0);
        size_t i=0;
        ...
        while(i<pent.size()){
            ...
            const uint32_t sz=(uint32_t)(j-i);
            if(sz>1) sizes.push_back(sz);
            for(size_t q=i;q<j;++q) bucketSizeOf[(uint32_t)(pent[q]&0xFFFFFFFFULL)]=sz;
            i=j;
        }
        if(!sizes.empty()){
            std::sort(sizes.begin(),sizes.end());
            const size_t idx=(size_t)(0.995*(double)(sizes.size()-1));  // 99.5th percentile
            repeatExcludeThresh=std::max(sizes[idx],(uint32_t)64);      // never exclude below 64 -- small buckets are cheap regardless
        }
        fprintf(stderr,"[REPEAT] indexed=%zu distinct-repeated-buckets=%zu repeatExcludeThresh=%u\n",
                pent.size(),sizes.size(),repeatExcludeThresh);
    }
    buildPref(false);   // pass 2: same reads/admit, now actually excludes the repeat-family tail
    lap("prefix seed index (pass 2, repeat-excluded)");
    ...
    auto sweep=[&](){
        ...
        for(uint32_t i=0;i<n;++i)
            if(admit[i]&&rlen[i]>=sweep_minov&&bucketSizeOf[i]<repeatExcludeThresh)
                tails.push_back(i);
