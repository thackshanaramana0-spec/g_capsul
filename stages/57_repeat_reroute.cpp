...
int main(int argc,char** argv){
    ...
    size_t n_matched=0;
    {
        ...
        const uint32_t MAXDIV=(getenv("MAXMAP_DIV")?atoi(getenv("MAXMAP_DIV")):3);
        const uint32_t MAXMAP=(uint32_t)(Lmax/MAXDIV);   // ReadsMatchers.cpp:700 -- MAXMAP_DIV env override for stage 54 sweep
    ...
    // main_pg_end must be known before STAGE 57's reroute pass runs, so it is
    // hoisted here (it does not change value -- pg.size() at this point is
    // still the main-pg boundary; the second pg is appended after this block).
    const size_t main_pg_end = pg.size();

    // ── STAGE 57: reroute reads mapped into a near-repeat confusion zone ────
    // Measured (56_het_probe.cpp analysis): 31% of the mismatch stream sits
    // at only ~166K pg positions where 3+ INDEPENDENT reads agree on the same
    // alternate base -- far beyond what independent sequencing error could
    // produce. Median gap between such sites is 1 base (95% within 50 bases
    // of each other): this is not sparse heterozygous-SNP structure, it is
    // dense runs -- the signature of reads from a near-identical repeat copy
    // being force-mapped (DIV=1's permissive budget) against a DIFFERENT,
    // similar-but-not-identical copy already in the pg.
    //
    // Hypothesis: 37,525 reads (14.3% of mapped reads) carry at least one
    // mismatch at such a site, costing ~196,540 B in mismatch coding. If
    // these reads are rerouted to the leftover/survivor path INSTEAD of
    // being force-mapped, they should overlap EACH OTHER (being copies of the
    // same alternate locus) and assemble into a compact second consensus,
    // which then MEM-matches the main pg -- cheaper than paying per-base
    // mismatch cost for a placement that was already wrong.
    //
    // This is a REAL TEST, not an estimate: naive back-of-envelope pricing
    // (37,525 reads x the OLD survivor average of ~90 B/read, measured on a
    // population that genuinely matched nothing) predicts a 3.4 MB LOSS --
    // wildly worse than the current 196,540 B. That estimate is almost
    // certainly wrong for THIS population (they should self-assemble far
    // more cheaply than reads that match nothing), but it must be measured,
    // not assumed, given the size of the risk if wrong.
    size_t rerouted=0;
    {
        std::unordered_map<uint64_t,uint32_t> siteCount; // pgpos*4+altcode -> count
        auto code2=[](char c)->int{ return c=='A'?0:c=='C'?1:c=='G'?2:3; };
        ...
        // Pass 1: count independent reads per (pgpos, alt-base).
        for(uint32_t i=0;i<n;++i){
            if(readMM[i]==255) continue;
            const uint32_t RL=rlen[i];
            int64_t q; bool rc=prc[i];
            if(rc) q=(int64_t)main_pg_end-(int64_t)ppos[i]-(int64_t)RL; else q=(int64_t)ppos[i];
            if(q<0||q+RL>main_pg_end) continue;
            for(uint32_t j=0;j<RL;++j){
                char refc,obsc; uint32_t pgpos;
                if(!rc){ refc=pg[q+j]; obsc=rbase(i,j); pgpos=(uint32_t)(q+j); }
                else   { refc=pg[q+RL-1-j]; obsc=comp(rbase(i,j)); pgpos=(uint32_t)(q+RL-1-j); }
                if(refc==obsc) continue;
                const uint64_t key=((uint64_t)pgpos<<2)|(uint64_t)code2(obsc);
                ++siteCount[key];
            }
        }
        // Pass 2: unmap any read touching a site with >=3 independent agreements.
        for(uint32_t i=0;i<n;++i){
            if(readMM[i]==255||!matched[i]) continue;
            const uint32_t RL=rlen[i];
            int64_t q; bool rc=prc[i];
            if(rc) q=(int64_t)main_pg_end-(int64_t)ppos[i]-(int64_t)RL; else q=(int64_t)ppos[i];
            if(q<0||q+RL>main_pg_end) continue;
            bool hit=false;
            for(uint32_t j=0;j<RL && !hit;++j){
                char refc,obsc; uint32_t pgpos;
                if(!rc){ refc=pg[q+j]; obsc=rbase(i,j); pgpos=(uint32_t)(q+j); }
                else   { refc=pg[q+RL-1-j]; obsc=comp(rbase(i,j)); pgpos=(uint32_t)(q+RL-1-j); }
                if(refc==obsc) continue;
                const uint64_t key=((uint64_t)pgpos<<2)|(uint64_t)code2(obsc);
                if(siteCount[key]>=3) hit=true;
            }
            if(hit){ matched[i]=0; readMM[i]=255; --n_matched; ++rerouted; }
        }
        fprintf(stderr,"[REROUTE] reads unmapped from repeat-confusion sites: %zu\n",rerouted);
    }
    ...
    if(getenv("DUMP_MM")){
        ...
        FILE* fr=fopen("mm_ref.bin","wb"), *fo=fopen("mm_obs.bin","wb"),
             ...
             *fg=fopen("mm_pgpos.bin","wb"),   // STAGE 56: absolute pg coordinate, 4B LE
             *frid=fopen("mm_rid.bin","wb");   // which read this mismatch came from, 4B LE
        size_t emitted=0;
        for(uint32_t i=0;i<n;++i){
            if(readMM[i]==255) continue;
            const uint32_t RL=rlen[i];
        ...
        fclose(fr); fclose(fo); fclose(fp); fclose(fx); fclose(fg); fclose(frid);
