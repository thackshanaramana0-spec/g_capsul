...
int main(int argc,char** argv){
    ...
    // ── STAGE 39: sort-merge sweep, replacing the hash-probe sweep ──────────
    // PgRC2's division runs in 1186 ms with only a PARALLEL SORT
    // (GreedySwipingPackedOverlapPseudoGenomeGenerator.cpp:6-10, pstld /
    // __gnu_parallel) and a serial merge loop, while ours takes 1.3-1.9 s fully
    // threaded. They beat us using less parallelism, so the deficit is
    // algorithmic and no amount of threading fixes it.
    //
    // Their overlapSortedReadsAndMergeSortSuffixes is a two-list merge:
    //   - reads sorted by full sequence are ALREADY sorted by every prefix
    //     length, so the prefix side is built once and never rebuilt;
    //   - as L drops by one the suffix key loses its FIRST character, so a
    //     stable bucket by that character, concatenated A/C/G/T, restores sorted
    //     order in O(n) -- no re-sort per level;
    //   - two sorted lists then merge in one linear pass.
    //
    // Ours probed a 40 MB hash table once per open tail per level: 20.9M random
    // accesses, nearly all cache misses. Same lemma, opposite memory behaviour.
    //
    // Not byte-identical to the hash sweep, and it cannot be: ties are broken in
    // sorted order rather than hash-bucket order, so a read with several equal
    // candidates may link to a different one. Sizes are reported so the effect
    // is measured rather than assumed. Comparison counts are also printed --
    // deterministic, and therefore trustworthy on a machine where wall clock is
    // not (another session runs concurrently here).
    auto sweep_merge=[&](){
        links=0; probes=0;
        std::fill(nxt.begin(),nxt.end(),NONE); std::fill(prv.begin(),prv.end(),NONE);
        std::fill(ovl.begin(),ovl.end(),0);
        for(uint32_t i=0;i<n;++i){ ch_h[i]=i; ch_t[i]=i; }

        // compare L bases: a from ao against b from bo. 2-bit big-endian packing
        // makes a numeric word compare equal to a lexicographic base compare.
        auto cmpk=[&](uint32_t a,uint32_t ao,uint32_t b,uint32_t bo,uint32_t L)->int{
            uint64_t pa=woff[a]*32ULL+ao, pb=woff[b]*32ULL+bo;
            while(L>=32){
                const uint64_t x=w32(pa), y=w32(pb);
                if(x!=y) return x<y?-1:1;
                pa+=32; pb+=32; L-=32; ++probes;
            }
            if(L){
                const uint64_t m=~0ULL<<(2*(32-L));
                const uint64_t x=w32(pa)&m, y=w32(pb)&m;
                ++probes;
                if(x!=y) return x<y?-1:1;
            }
            return 0;
        ...
        // ── rolling keys ────────────────────────────────────────────────────
        // Comparing through cmpk means two random loads into the 34 MB packed
        // read array for every comparison; at ~160M comparisons that is ~320M
        // cache misses and it is the whole cost -- 10.5 s serial, where fixing
        // the allocations changed nothing (10.81 -> 10.52 s).
        //
        // Instead give each read a 32-base key in a compact array. The prefix
        // key is the read's first 32 bases and never changes (only its mask
        // does). The suffix key loses its first base per level, so it rolls:
        // key = (key << 2) | base_at(off+32) -- one base read per read per
        // level, against one per comparison. The merge then compares uint64s
        // sequentially in a 6.8 MB array.
        std::vector<uint64_t> skey(n,0), pkey(n,0);
        for(uint32_t i=0;i<n;++i){
            if(!admit[i]) continue;
            pkey[i]=(rlen[i]>=32)?w32(woff[i]*32ULL) : (w32(woff[i]*32ULL)&(~0ULL<<(2*(32-rlen[i]))));
        }
        auto baseAt=[&](uint32_t a,uint32_t o)->uint64_t{
            return (o<rlen[a]) ? ((w32(woff[a]*32ULL+o)>>62)&3ULL) : 0ULL; };
        auto buildSkey=[&](uint32_t a,uint32_t L){
            const uint32_t off=(uint32_t)rlen[a]-L;
            uint64_t k=0;
            for(uint32_t t=0;t<32;++t) k=(k<<2)|baseAt(a,off+t);
            skey[a]=k; };

        // prefix side: sorted once by full sequence
        std::vector<uint32_t> pre;
        pre.reserve(n);
        for(uint32_t i=0;i<n;++i) if(admit[i]&&rlen[i]>=sweep_minov) pre.push_back(i);
        std::sort(pre.begin(),pre.end(),[&](uint32_t x,uint32_t y){
            if(pkey[x]!=pkey[y]) return pkey[x]<pkey[y];
            const uint32_t L=(rlen[x]<rlen[y])?rlen[x]:rlen[y];
            const int c=cmpk(x,0,y,0,L);
            return c ? c<0 : rlen[x]<rlen[y];
        });

        // suffix side at the first level, sorted explicitly; maintained by
        // bucketing thereafter
        const uint32_t L0=Lmax-1;
        std::vector<uint32_t> suf;
        suf.reserve(pre.size());
        for(uint32_t i=0;i<n;++i) if(admit[i]&&rlen[i]>=L0&&rlen[i]>=sweep_minov) suf.push_back(i);
        for(uint32_t a:suf) buildSkey(a,L0);
        std::sort(suf.begin(),suf.end(),[&](uint32_t x,uint32_t y){
            if(skey[x]!=skey[y]) return skey[x]<skey[y];
            return cmpk(x,(uint32_t)rlen[x]-L0,y,(uint32_t)rlen[y]-L0,L0)<0;
        });
        std::vector<uint32_t> bk[4], arrive, tmp, mbuf;
        mbuf.reserve(n);
        for(int q=0;q<4;++q) bk[q].reserve(n/2);



        for(uint32_t L=L0; L>=sweep_minov && L>=1; --L){
            // reads that only become long enough now (fixed-length input: none)
            arrive.clear();
            for(uint32_t i=0;i<n;++i)
                if(admit[i]&&rlen[i]==L&&L>=sweep_minov&&nxt[i]==NONE) arrive.push_back(i);
            if(!arrive.empty()){
                std::sort(arrive.begin(),arrive.end(),[&](uint32_t x,uint32_t y){
                    return cmpk(x,(uint32_t)rlen[x]-L,y,(uint32_t)rlen[y]-L,L)<0; });
                tmp.clear(); tmp.reserve(suf.size()+arrive.size());
                size_t i1=0,i2=0;
                while(i1<suf.size()&&i2<arrive.size()){
                    const uint32_t a=suf[i1],b=arrive[i2];
                    tmp.push_back(cmpk(a,(uint32_t)rlen[a]-L,b,(uint32_t)rlen[b]-L,L)<=0?suf[i1++]:arrive[i2++]);
                }
                while(i1<suf.size())    tmp.push_back(suf[i1++]);
                while(i2<arrive.size()) tmp.push_back(arrive[i2++]);
                suf.swap(tmp);
            }
            // drop closed tails and claimed prefixes
            { size_t w=0; for(uint32_t a:suf) if(nxt[a]==NONE&&rlen[a]>=L) suf[w++]=a; suf.resize(w); }
            { size_t w=0; for(uint32_t b:pre) if(prv[b]==NONE&&rlen[b]>=L) pre[w++]=b; pre.resize(w); }
            if(suf.empty()||pre.empty()) break;

            // key compare first; only fall through to the packed reads when the
            // first 32 bases tie, which needs L>32 to matter at all
            const uint64_t KM=(L>=32)?~0ULL:(~0ULL<<(2*(32-L)));
            auto kcmp=[&](uint32_t a,uint32_t b)->int{
                const uint64_t x=skey[a]&KM, y=pkey[b]&KM;
                if(x!=y) return x<y?-1:1;
                if(L<=32) return 0;
                return cmpk(a,(uint32_t)rlen[a]-L,b,0,L); };

            // ── merge, searched in parallel and committed serially ──────────
            // Same split that made stage 19 work: the search reads only state
            // fixed for the level, while the commit (prv, cycle) must stay
            // ordered. Both lists are sorted, so a thread's slice of `suf` maps
            // to a slice of `pre` by binary search -- the walk is monotone, so
            // chunk boundaries are exact.
            {
                unsigned T=std::thread::hardware_concurrency(); if(!T) T=1;
                if(suf.size()<8192) T=1;
                struct Hit { uint32_t x, j, j2; };
                std::vector<std::vector<Hit>> hits(T);
                std::vector<std::thread> th; th.reserve(T);
                const size_t span=(suf.size()+T-1)/T;
                for(unsigned t=0;t<T;++t){
                    const size_t lo=(size_t)t*span, hi=std::min(suf.size(),lo+span);
                    if(lo>=hi) break;
                    th.emplace_back([&,lo,hi,t]{
                        // first prefix not less than this slice's first suffix
                        size_t j=0,a0=0,b0=pre.size();
                        while(a0<b0){ const size_t m=(a0+b0)/2;
                                      if(kcmp(suf[lo],pre[m])>0) a0=m+1; else b0=m; }
                        j=a0;
                        size_t i=lo;
                        auto& out=hits[t];
                        while(i<hi&&j<pre.size()){
                            const int c=kcmp(suf[i],pre[j]);
                            if(c<0){ ++i; continue; }
                            if(c>0){ ++j; continue; }
                            size_t i2=i+1;
                            while(i2<hi&&kcmp(suf[i2],pre[j])==0) ++i2;
                            size_t j2=j+1;
                            while(j2<pre.size()&&kcmp(suf[i],pre[j2])==0) ++j2;
                            for(size_t ii=i;ii<i2;++ii)
                                out.push_back({suf[ii],(uint32_t)j,(uint32_t)j2});
                            i=i2; j=j2;
                        }
                    });
                }
                for(auto& x:th) x.join();
                for(auto& v:hits) for(const Hit& h:v){
                    const uint32_t x=h.x;
                    if(nxt[x]!=NONE) continue;
                    for(uint32_t jj=h.j;jj<h.j2;++jj){
                        const uint32_t y=pre[jj];
                        if(y==x||prv[y]!=NONE||ch_h[x]==y) continue;
                        nxt[x]=y; prv[y]=x; ovl[x]=L;
                        const uint32_t hh=ch_h[x],tt=ch_t[y]; ch_t[hh]=tt; ch_h[tt]=hh; ++links;
                        break;
                    }
                }
            }

            // ── O(n) re-sort for the next level ─────────────────────────────
            // The key loses its FIRST base, so bucketing by that base leaves
            // each bucket already sorted by the new key -- but the buckets must
            // then be MERGED, not concatenated. Concatenating rebuilds sorted-by
            // (dropped_base, rest), which is the OLD key: a no-op. That was the
            // first version's bug, and it showed up as 41,564 links at L=149 and
            // almost none at any level below, because every level after the
            // first was merging an order that had never actually changed.
            //
            // This is the machinery PgRC2 keeps in ssiOrder + updateSuffixQueue,
            // which reads as bookkeeping and is in fact the algorithm.
            if(L>sweep_minov){
                const uint32_t NL=L-1;
                for(int q=0;q<4;++q) bk[q].clear();
                for(uint32_t a:suf) bk[(skey[a]>>62)&3ULL].push_back(a);
                // One 4-way merge straight into a preallocated buffer. Three
                // pairwise merges with vector locals cost two intermediate
                // copies AND a fresh allocation on each of ~134 levels, which is
                // what made the first correct version 7x slower than the hash
                // sweep despite doing 8x fewer comparisons. The comparison count
                // was never the cost; the memory traffic was.
                // roll every surviving key one base before merging on it
                for(uint32_t a:suf)
                    skey[a]=(skey[a]<<2)|baseAt(a,(uint32_t)rlen[a]-L+32);
                const uint64_t KM2=(NL>=32)?~0ULL:(~0ULL<<(2*(32-NL)));
                auto key2=[&](uint32_t x,uint32_t y)->int{
                    const uint64_t px=skey[x]&KM2, py=skey[y]&KM2;
                    if(px!=py) return px<py?-1:1;
                    if(NL<=32) return 0;
                    return cmpk(x,(uint32_t)rlen[x]-NL,y,(uint32_t)rlen[y]-NL,NL); };
                size_t p0=0,p1=0,p2=0,p3=0;
                mbuf.clear();
                while(true){
                    int best=-1; uint32_t bv=0;
                    if(p0<bk[0].size()){ best=0; bv=bk[0][p0]; }
                    if(p1<bk[1].size()&&(best<0||key2(bk[1][p1],bv)<0)){ best=1; bv=bk[1][p1]; }
                    if(p2<bk[2].size()&&(best<0||key2(bk[2][p2],bv)<0)){ best=2; bv=bk[2][p2]; }
                    if(p3<bk[3].size()&&(best<0||key2(bk[3][p3],bv)<0)){ best=3; bv=bk[3][p3]; }
                    if(best<0) break;
                    mbuf.push_back(bv);
                    if(best==0) ++p0; else if(best==1) ++p1; else if(best==2) ++p2; else ++p3;
                }
                suf.swap(mbuf);
            }
        }
    ...
    const bool MSWEEP=(getenv("MERGE_SWEEP")!=nullptr);
    auto sweep_any=[&](){ if(MSWEEP) sweep_merge(); else sweep(); };
