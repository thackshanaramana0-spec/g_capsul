...
int main(int argc,char** argv){
    ...
    // ── STAGE 31 probe: how many links is RC orientation actually worth? ─────
    // "reverse complement in the greedy" is refuted twice in this file (stage 04,
    // and again in a second architecture). Both refutations predate MEM removal,
    // exactly like stage 14's did -- and stage 14's conclusion was explicitly
    // flagged as needing a re-test on top of stage 16 for that reason.
    //
    // Rather than build the whole thing on a hunch, this counts the headroom:
    // index every read's RC prefix as well, then for each tail that finished
    // round 1 WITHOUT a successor, ask whether its suffix would have matched
    // some read's RC prefix. That is an upper bound on the extra links RC could
    // supply, measured before committing to the orientation bookkeeping that a
    // real implementation needs.
    if(getenv("RC_PROBE")){
        std::unordered_map<uint64_t,std::vector<uint32_t>> rcpref;
        rcpref.reserve(n*2);
        std::string tmp;
        for(uint32_t i=0;i<n;++i){
            if(rlen[i]<SW) continue;
            tmp.assign(rlen[i],'A'); runp(i,&tmp[0]); rc_inplace(tmp);
            uint64_t k; if(packSW(tmp.data(),k)) rcpref[k].push_back(i);
        }
        size_t open_tails=0, rc_hits=0, fwd_would=0;
        std::string sa;
        for(uint32_t a=0;a<n;++a){
            if(!admit[a]||nxt[a]!=NONE) continue;   // chain tails only
            ++open_tails;
            // longest overlap this tail could still take, at the round-2 floor
            bool hit=false, fhit=false;
            for(uint32_t L=rlen[a]-1; L>=MINOV && L>=SW && !hit; --L){
                const uint32_t off=(uint32_t)rlen[a]-L;
                if(off+SW>rlen[a]) continue;
                const uint64_t k=rseed(a,off,SW);
                auto it=rcpref.find(k);
                if(it!=rcpref.end()){
                    sa.assign(rlen[a],'A'); runp(a,&sa[0]);
                    for(uint32_t b:it->second){
                        if(b==a||!admit[b]||rlen[b]<L) continue;
                        if(prv[b]!=NONE) continue;      // only a chain HEAD is worth joining
                        tmp.assign(rlen[b],'A'); runp(b,&tmp[0]); rc_inplace(tmp);
                        if(memcmp(sa.data()+off,tmp.data(),L)==0){ hit=true; break; }
                    }
                }
                if(!fhit){ auto f=pref.find(k); if(f!=pref.end()) fhit=true; }
            }
            if(hit) ++rc_hits;
            if(fhit) ++fwd_would;
        }
        fprintf(stderr,"[RC-PROBE] chain tails after round 2: %zu\n",open_tails);
        fprintf(stderr,"[RC-PROBE]   would link via RC: %zu (%.1f%%)\n",
                rc_hits,open_tails?100.0*rc_hits/open_tails:0.0);
        fprintf(stderr,"[RC-PROBE]   forward seed existed anyway: %zu\n",fwd_would);
    }
