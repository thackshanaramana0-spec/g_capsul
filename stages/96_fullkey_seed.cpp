...
int main(int argc,char** argv){
    ...
    // STAGE 96 -- real root cause found (not the sampling-bias guess stage 95
    // tested and disproved by direct measurement: striding was marginally
    // WORSE than truncation at the same budget, showing repeat buckets fail
    // rcmp almost uniformly, not from a positional scan bias). The actual
    // bug: `pent`'s bucketing key was `(uint32_t)rseed(i,0,SW)` -- rseed
    // returns SW bases packed into a 64-bit value (up to 32 bases, base 0 in
    // the HIGH bits), but casting the result to uint32_t keeps only the LOW
    // 32 bits -- i.e. only the LAST 16 bases of whatever SW-base seed was
    // requested, REGARDLESS of SW. So the seed INDEX key was always a fixed
    // 16-mer (4^16 possibilities), even when SW=32 was configured -- SW only
    // ever affected the later rcmp verification length, never the index
    // lookup's actual selectivity. On a repeat-dense genome (T. cacao), many
    // unrelated reads genuinely share the same 16-mer by chance, producing
    // the huge collision buckets stage 94 measured directly (avg scan length
    // 11-62 vs E. coli's 0.12).
    //
    // Fix: use the FULL SW-base (up to 64-bit) seed as the bucketing key
    // instead of truncating it -- a real, much more selective index (up to
    // 4^32 possibilities at SW=32) that should shrink these buckets at the
    // source. Requires restructuring `pent` (previously packed key+read_id
    // into one uint64_t, which no longer fits a 64-bit key) into two
    // parallel arrays sorted together by the full key.
    std::vector<uint64_t> pkey;   // full SW-base seed, sorted
    std::vector<uint32_t> pval;   // read id, same order as pkey
    std::vector<uint32_t> ptab;
    size_t pmask=0;
    auto pmix=[](uint64_t x){ x^=x>>33; x*=0xff51afd7ed558ccdULL; x^=x>>33; return x; };
    auto buildPref=[&](bool useAdmit){
        pkey.clear(); pval.clear(); pkey.reserve(n); pval.reserve(n);
        for(uint32_t i=0;i<n;++i){
            if(useAdmit && !admit[i]) continue;
            if(rlen[i]<SW) continue;
            pkey.push_back(rseed(i,0,SW));   // full seed, no truncation
            pval.push_back(i);
        }
        std::vector<uint32_t> idx(pkey.size());
        for(size_t i=0;i<idx.size();++i) idx[i]=(uint32_t)i;
        std::sort(idx.begin(),idx.end(),[&](uint32_t a,uint32_t b){ return pkey[a]<pkey[b]; });
        std::vector<uint64_t> pkey2(pkey.size()); std::vector<uint32_t> pval2(pval.size());
        for(size_t i=0;i<idx.size();++i){ pkey2[i]=pkey[idx[i]]; pval2[i]=pval[idx[i]]; }
        pkey.swap(pkey2); pval.swap(pval2);
        size_t sz=1; while(sz<pkey.size()*2+1) sz<<=1;
        pmask=sz-1; ptab.assign(sz,UINT32_MAX);
        for(size_t i=0;i<pkey.size();++i){
            if(i && pkey[i-1]==pkey[i]) continue;
            size_t h=pmix(pkey[i])&pmask;
            while(ptab[h]!=UINT32_MAX) h=(h+1)&pmask;
            ptab[h]=(uint32_t)i;
        }
    };
    auto pfind=[&](uint64_t key)->uint32_t{
        size_t h=pmix(key)&pmask;
        while(ptab[h]!=UINT32_MAX){
            if(pkey[ptab[h]]==key) return ptab[h];
            h=(h+1)&pmask;
        }
        return UINT32_MAX;
    };
    auto pbucketEnd=[&](uint32_t pix)->uint32_t{
        auto it=std::upper_bound(pkey.begin()+pix,pkey.end(),pkey[pix]);
        return (uint32_t)(it-pkey.begin());
    };
    buildPref(false);
    ...
    for(uint32_t L=Lmax-1; L>=sweep_minov && L>=SW; --L){
        ...
        ccnt.assign(w,0);
        {
            ...
            for(long long ii=0;ii<(long long)w;++ii){
                ...
                uint8_t c=0;
                uint32_t scanlen=0;
                // STAGE 96: plain truncated scan over the full-key index --
                // stage 95's striding is dropped, disproved by direct
                // measurement (marginally worse than truncation, same
                // budget). This isolates the real fix (full-key bucketing,
                // above) from the disproved sampling-order idea.
                const uint32_t bend=std::min(pbucketEnd(pix),pix+MAXSCAN);
                {
                    for(uint32_t q=pix;q<bend;++q){
                        ++scanlen;
                        const uint32_t b=pval[q];
                        if(b==a) continue;
        ...
        for(size_t i=0;i<w;++i){
            ...
            if(pix==UINT32_MAX) continue;
            uint8_t seen=0;
            const uint32_t qend2=std::min(pbucketEnd(pix),pix+MAXSCAN);
            for(uint32_t q=pix;q<qend2;++q){
                const uint32_t b=pval[q];
                if(b==a) continue;
                if(!admit[b]) continue;
                if(rlen[b]<L) continue;
    ...
    { std::vector<uint64_t>().swap(pkey); std::vector<uint32_t>().swap(pval); std::vector<uint32_t>().swap(ptab); }
#if defined(__GLIBC__)
    malloc_trim(0);
#endif
