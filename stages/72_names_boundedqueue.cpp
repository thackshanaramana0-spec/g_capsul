...
struct BoundedQueue {
    ...
    void push(T v){
        ...
        cv_push.wait(lk,[&]{ return q.size()<cap; }); // BACKPRESSURE: blocks reader once queue full -- this bounds memory
...
int main(int argc,char**argv){
    if(argc<2){ fprintf(stderr,"usage: %s names.txt [block_size=100000] [nthreads=nproc] [queue_cap=2*nthreads]\n",argv[0]); return 1; }
