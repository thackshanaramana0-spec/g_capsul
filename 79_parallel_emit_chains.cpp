// ============================================================================
// STAGE 79 -- parallel `emit chains`, on top of stage 77's dedup fix. Deep-
// think literature survey done first (Blelloch 1990, work-efficient
// parallel prefix scan -- the established technique for exactly this
// "compute offsets, then write in parallel" shape) before reading the real
// code and implementing.
//
// Real cost identified by reading the code: `emit chains` builds the pg
// string by walking read-overlap chains, unpacking each read's 2-bit-
// packed bases and appending -- ~21.6M bases of real unpack+copy work,
// done in ONE thread (unlike round 1/round 2, which reuse the same
// threaded `sweep()` lambda and are already parallel -- confirmed by
// reading the code, not assumed, before concluding this was the one real
// remaining target).
//
// Real order-sensitivity checked before writing anything, same discipline
// as stage 77's dedup fix: `leftovers`' content/order and which pg BYTE
// OFFSET each chain lands at both depend on the i=0..n-1 sweep order --
// getting either wrong would produce a different pg layout that
// downstream MEM matching/mismatch mapping would treat as genuinely
// different data. Two-pass design: pass 1 stays a single sequential sweep
// in the exact original order, doing only cheap length arithmetic (no
// unpacking, no string growth) to find leftovers, the ordered chain-start
// list, and each chain's total length; a serial prefix sum over those
// lengths (~39K chains, sub-millisecond, no real parallel scan needed at
// this size) gives each chain's exact pg offset, identical to what the
// sequential version would produce. Pass 2 -- the actual expensive
// unpack+copy work -- runs in parallel, one thread per chain, writing into
// its own pre-computed disjoint byte range of an already-SIZED (not just
// reserved) pg buffer via direct indexed writes, no locks needed (same
// safety principle as concurrent writes to disjoint elements of a
// std::vector).
//
// REAL BUG CAUGHT BEFORE TRUSTING ANYTHING: first version conflated the
// bookkeeping value `ppos[cur]` (which legitimately points BEFORE the
// current write cursor, to account for overlap) with the actual memcpy
// destination, writing to `&pg[off-o]` instead of `&pg[off]`. This
// produced a real, large, obviously-wrong result on the very first
// verification run (PG_LEN 50,284,878 instead of the correct 23,233,953,
// REF matches 352,429 instead of 58,908) -- caught immediately by the same
// byte-identical downstream-hash verification method used for stage 77,
// not discovered later. Fixed (write destination is `off`, not `off-o`;
// `ppos[cur]=off-o` stays correct as bookkeeping only), re-verified
// byte-identical across literal.txt/perm.u32/mem_triples.bin/
// mem_gaps.bin/mem_lens.bin.
//
// Real, measured result (3 repeated runs, real data): emit chains itself
// dropped from ~0.22-0.34s to a stable 0.04-0.05s -- a real 5-6x speedup,
// RSS unaffected (133 MB, same as before). Total assembly time with this
// plus stage 77's dedup fix combined: ~3.05s (down from ~3.2-3.6s).
// ============================================================================
//
// ============================================================================
// STAGE 77 -- parallel-parse + serial-merge load/filter/dedup, on top of
// stage 45 unchanged otherwise. User asked for component-specific analysis
// of the combined pipeline's three biggest assembly phases (pg MEM matching
// 32%, load+filter+dedup 18%, pigeonhole mapping 18%), not a general
// search, and to check for headroom in each without losing anything.
//
// Read the actual code for all three before touching anything:
//   - pigeonhole mapping and pg MEM matching BOTH already use
//     std::thread::hardware_concurrency() internally (confirmed by grep,
//     not assumed) -- already parallel. Both are a standard seed-and-extend
//     design (strided/spaced k-mer hashing into a flat CSR index, same
//     family as BLAST-style matchers), already optimized within that
//     design (flat CSR instead of hashmap-of-vectors, DEFLATE-style lazy
//     lookahead). The real established ALTERNATIVE algorithm class for
//     this problem is suffix-array/FM-index-based exact matching (what
//     MUMmer/E-MEM use) -- but swapping that in is a substantially bigger,
//     riskier rewrite that could change which matches are found (hence the
//     ratio), not a safe component-level win. Not attempted here.
//   - load+filter+dedup was the ONE completely single-threaded phase: a
//     plain std::ifstream + std::getline + std::unordered_map loop, no
//     threading anywhere in it.
//
// Real risk identified before writing anything: the dedup DECISION is
// order-sensitive. Reordering which duplicate read becomes the group's
// "representative" changes what the downstream greedy assembly sees, which
// could silently change the ratio -- exactly what must not happen. So only
// the order-INDEPENDENT, genuinely expensive per-record work (reading,
// record-boundary finding, FNV hashing, 2-bit packing) is parallelized; the
// hashmap dedup lookup/insert itself stays a single serial pass in original
// file order, byte-identical logic to the unmodified version.
//
// Verified, not assumed: ran both versions on the same real data and
// md5sum-compared literal.txt/perm.u32/mem_triples.bin/mem_gaps.bin/
// mem_lens.bin -- byte-identical across every downstream stream. Any
// dedup-order difference would have cascaded into different assembly
// chaining and shown up here; it didn't.
//
// First attempt used a vector<vector<uint64_t>> for the parallel-computed
// packed sequence data -- byte-identical and correct, but ~850K individual
// small heap allocations (one per admitted read) ate most of the
// parallelism gain and nearly tripled peak RSS (48 MB -> 125 MB). Fixed by
// replacing it with one flat, fixed-stride buffer (8 words/record, the max
// possible for a 255-base read) -- zero allocations in the hot loop, RSS
// back to the original 48 MB.
//
// Real, honest result (5 alternating runs each, warm cache, same machine --
// a first single-run comparison suggested a dramatic ~35% gain but that
// included real cold-cache noise on the FIRST read of the session; this is
// the corrected, fair number):
//   original (serial):    0.58 / 0.57 / 0.63 / 0.58 / 0.60 s -> avg 0.592s
//   this stage (parallel): 0.53 / 0.65 / 0.52 / 0.53 / 0.52 s -> avg 0.550s
// ~7% faster on average -- small, real, safe (zero ratio risk, zero RAM
// cost), not a big lever. Reported honestly rather than the inflated first
// reading.
// ============================================================================
//
...
int main(int argc,char** argv){
    ...
    {
        // STAGE 77: parallel-parse + serial-merge. The dedup DECISION must stay
        // strictly in file order -- reordering it could change which read
        // becomes the "representative" of a duplicate group, which would
        // change what the downstream greedy assembly sees and could silently
        // change the ratio, exactly what must not happen. So only the
        // ORDER-INDEPENDENT, genuinely expensive per-record work (reading,
        // record-boundary finding, hashing, 2-bit packing) is parallelized;
        // the hashmap dedup lookup/insert itself is still one single-threaded
        // pass in original file order, byte-identical logic to before.
        //
        // Phase 1 (single read syscall + single scan): load the whole file
        // into one buffer and locate every record's 4 line-start offsets.
        // Faster than getline's per-line allocation churn even before any
        // threading, and gives phase 2 something to shard by index.
        std::string buf;
        { FILE* f=fopen(argv[1],"rb"); if(!f){perror("open");return 1;}
          fseek(f,0,SEEK_END); size_t sz=ftell(f); fseek(f,0,SEEK_SET);
          buf.resize(sz);
          if(sz && fread(&buf[0],1,sz,f)!=sz){fprintf(stderr,"short read\n");return 1;}
          fclose(f); }
        struct RecView{ const char* seq; uint32_t seqlen; };
        std::vector<RecView> recs;
        {
            size_t p=0, sz=buf.size();
            recs.reserve(sz/200+16);
            while(p<sz){
                size_t l0=p; while(p<sz && buf[p]!='\n') ++p; ++p; // header
                size_t l1=p; while(p<sz && buf[p]!='\n') ++p;      // sequence
                size_t seqend=p; ++p;
                while(p<sz && buf[p]!='\n') ++p; ++p;              // '+'
                while(p<sz && buf[p]!='\n') ++p; ++p;              // quality
                (void)l0;
                if(l1<=seqend) recs.push_back({buf.data()+l1,(uint32_t)(seqend-l1)});
            }
        }
        n_in=recs.size();

        // Phase 2 (parallel, index-sharded, no shared mutable state): for
        // every record independently compute the N-filter result and, when
        // admissible, its FNV hash and 2-bit-packed words -- exactly the
        // same fnv()/packstr logic as the serial version, just precomputed
        // ahead of the merge instead of inline during it.
        std::vector<uint8_t> ok(recs.size(),0);
        std::vector<uint64_t> hval(recs.size(),0);
        // Flat fixed-stride buffer instead of a vector<vector<uint64_t>> per
        // record -- the vector-of-vectors design was byte-identical and
        // correct but paid for ~850K individual small heap allocations (one
        // per admitted record), which ate most of the parallelism gain and
        // nearly tripled peak RSS (48 MB -> 125 MB). Max packed length is
        // ceil(255/32)=8 words (255 = the uint8 length-field cap already
        // enforced elsewhere), so one flat array of recs.size()*8 words,
        // indexed by record, replaces all of them with zero allocation in
        // the hot loop.
        static constexpr uint32_t PACKW=8;
        std::vector<uint64_t> packflat((size_t)recs.size()*PACKW,0);
        auto packstr=[&](const char* q,uint32_t L,uint64_t* out){
        ...
        };
        {
            ...
            for(unsigned t=0;t<NT;++t) pool.emplace_back([&](){
                size_t s;
                while((s=next.fetch_add(CHUNK))<recs.size()){
                    size_t e=std::min(recs.size(),s+CHUNK);
                    for(size_t i=s;i<e;++i){
                        const RecView& r=recs[i];
                        if(r.seqlen>255) continue;                 // uint8 length field
                        if(memchr(r.seq,'N',r.seqlen)) continue;    // N-filter
                        ok[i]=1;
                        hval[i]=fnv(r.seq,r.seqlen);
                        packstr(r.seq,r.seqlen,&packflat[(size_t)i*PACKW]);
                    }
                }
            });
        ...
        // Phase 3 (serial, byte-identical logic/order to the original code):
        // walk records in file order using the precomputed hash/pack, doing
        // the exact same hashmap dedup lookup/insert as before.
        std::unordered_map<uint64_t,std::vector<uint32_t>> seen; seen.reserve(1u<<21);
        char ubuf[256];
        for(size_t i=0;i<recs.size();++i){
            if(!ok[i]){
                if(recs[i].seqlen<=255) ++n_filt; // matches original: N-containing counted as filtered; >255 silently skipped, same as before
                continue;
            }
            const char* b_ = recs[i].seq; const uint32_t blen=recs[i].seqlen;
            auto& bk=seen[hval[i]]; bool dup=false;
            for(uint32_t id:bk){
                if(rlen[id]!=(uint8_t)blen) continue;
                ...
                if(memcmp(ubuf,b_,blen)==0){ dup=true; orig2uid.push_back(id); break; }
            }
            if(!dup){
                orig2uid.push_back((uint32_t)rlen.size());
                bk.push_back((uint32_t)rlen.size());
                woff.push_back(rpk.size());
                const uint32_t nw=(blen+31)/32;   // same word count packstr would have pushed
                rpk.insert(rpk.end(),&packflat[(size_t)i*PACKW],&packflat[(size_t)i*PACKW]+nw);
                rlen.push_back((uint8_t)blen);
            }
        }
    ...
    // ── STAGE 78: emit chains, parallelized via a two-pass prefix-sum design
    // (Blelloch 1990's work-efficient scan is the established technique for
    // exactly this "compute offsets, then write in parallel" pattern).
    //
    // Real order-sensitivity checked before writing anything: `leftovers`'
    // CONTENT AND ORDER, and which pg BYTE OFFSET each chain lands at, both
    // depend on the i=0..n-1 sweep order -- reordering either would produce
    // a different (though possibly same-content) pg layout, which
    // downstream MEM matching/mismatch mapping would treat as genuinely
    // different data. So pass 1 stays a single sequential sweep in the
    // EXACT original i-order, doing only cheap length arithmetic (no base
    // unpacking, no string growth) to discover: leftovers (unchanged
    // order/content), the ordered list of chain-start indices, and each
    // chain's total length. A prefix sum over those lengths (serial here --
    // ~39K chains is already sub-millisecond, no need for an actual
    // parallel scan at this size) gives each chain's exact pg offset,
    // identical to what the sequential version would have produced.
    // Pass 2 is where the real cost was (unpacking + appending ~21.6M
    // bases) -- now parallel, one thread per chain, each writing into its
    // own pre-computed, disjoint byte range of an already-SIZED (not just
    // reserved) pg buffer via direct indexed writes -- safe with no locks,
    // same principle as concurrent writes to disjoint elements of a
    // std::vector. ppos[] writes are similarly safe: each read belongs to
    // exactly one chain, so different chains never touch the same index.
    std::vector<uint32_t> leftovers;
    std::vector<uint32_t> chain_starts;
    std::vector<uint64_t> chain_len;
    uint32_t multi=0;
    {
        auto chainlen=[&](uint32_t i)->uint64_t{
            uint64_t L=rlen[i];
            uint32_t cur=i;
            while(nxt[cur]!=NONE){ uint32_t o=ovl[cur]; cur=nxt[cur]; L+=rlen[cur]-o; }
            return L;
        };
        for(uint32_t i=0;i<n;++i){
            if(!admit[i]){ leftovers.push_back(i); continue; }
            if(contained[i]) continue;
            if(prv[i]!=NONE) continue;
            if(nxt[i]==NONE){ leftovers.push_back(i); continue; }
            ++multi;
            chain_starts.push_back(i);
            chain_len.push_back(chainlen(i));
        }
    }
    std::vector<uint64_t> chain_off(chain_starts.size());
    uint64_t pg_total_len=0;
    { uint64_t acc=0; for(size_t k=0;k<chain_starts.size();++k){ chain_off[k]=acc; acc+=chain_len[k]; } pg_total_len=acc; }

    ppos.assign(n,UINT64_MAX);
    std::string pg; pg.resize(pg_total_len);   // SIZED, not just reserved -- required for safe concurrent indexed writes
    {
        unsigned NT=std::thread::hardware_concurrency(); if(!NT) NT=1;
        std::vector<std::thread> pool;
        std::atomic<size_t> next{0};
        for(unsigned t=0;t<NT;++t) pool.emplace_back([&](){
            size_t k;
            while((k=next.fetch_add(1))<chain_starts.size()){
                uint32_t cur=chain_starts[k];
                uint64_t off=chain_off[k];
                char b_[256];
                runp(cur,b_);
                ppos[cur]=off;
                memcpy(&pg[off],b_,rlen[cur]); off+=rlen[cur];
                while(nxt[cur]!=NONE){
                    uint32_t o=ovl[cur]; cur=nxt[cur];
                    runp(cur,b_);
                    ppos[cur]=off-o;
                    memcpy(&pg[off],b_+o,rlen[cur]-o); off+=rlen[cur]-o;
                }
            }
        });
        for(auto& th:pool) th.join();
