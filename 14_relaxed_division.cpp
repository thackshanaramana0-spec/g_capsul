...
int main(int argc,char** argv){
    ...
    for(uint32_t L=Lmax-1; L>=sweep_minov && L>=SEED; --L){
        size_t w=0;
        for(uint32_t a:tails){
            ...
            if(off+SEED>A.size()) continue;
            tails[w++]=a;                                    // still open: keep
            if(!ok[a]){ ok[a]=pack(A.data()+off,seed[a])?1:0; if(!ok[a]) continue; }
            else{
                const int v=b2(A[off+SEED-1]);
                if(v<0){ ok[a]=0; continue; }
                seed[a]=((seed[a]<<2)|(uint64_t)v)&SEEDMSK;  // roll one base
            }
            ++probes;
    ...
    sweep_minov=SEED;                 // classifier: widest range the seed supports
    sweep();
    size_t both_sides=0;
    {
        // Relaxed admission. Requiring BOTH sides linked admits 67% of reads,
        // against the 83% PgRC2 keeps -- and their criterion takes a
        // coefficient, so it is evidently not the strict test. Strictness here
        // is self-defeating: every read held out of the pseudogenome is one
        // less place a later read can map to, so an over-tight division starves
        // the mapping stage and the survivors it fails to place cost a full
        // read length each. Admitting any read that linked at all (i.e. anything
        // that is not an isolated singleton) keeps the reads that tile, and
        // drops only those that overlap nothing.
        const bool strict = getenv("SCS_STRICT_DIVISION") != nullptr;
        std::vector<uint8_t> keep(n,0);
        for(uint32_t i=0;i<n;++i){
            const bool ok_read = strict ? (nxt[i]!=NONE && prv[i]!=NONE)
                                        : (nxt[i]!=NONE || prv[i]!=NONE);
            if(ok_read){ keep[i]=1; ++both_sides; }
        }
        admit.swap(keep);
    }
    ...
    size_t n_matched=0;
    {
        ...
        uint32_t SEEDW=(uint32_t)(Lmax/NPARTS); if(SEEDW>32) SEEDW=32;
        ...
        auto packW=[&](const char* p,uint64_t& out)->bool{
            ...
            for(uint32_t i=0;i<SEEDW;++i){ int v=b2(p[i]); if(v<0) return false;
                                          k=(k<<2)|(uint64_t)v; }
            out=k; return true; };
        fprintf(stderr,"  mapping: %u parts x %u bases (tolerating %d mismatches)\n",
                NPARTS,SEEDW,MAXMM);
