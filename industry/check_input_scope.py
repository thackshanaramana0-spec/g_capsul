...
WHY THIS EXISTS: `stages/106_inprocess.cpp` used to silently drop any read
over its structural length bound and complete normally, writing an empty
archive at exit code 0 -- a reviewer pointing this tool at unsupported input
saw success. The encoder itself now refuses loudly and unconditionally (see
`stages/106_inprocess.cpp`'s scope-check block, right after `int main`), so
THIS script cannot be bypassed into producing a false sense of safety -- it
...
ENCODER_SRC = os.path.join(REPO_ROOT, "stages", "106_inprocess.cpp")
...
def analyze_one(path, limits):
    ...
    if max_read_len is None:
        findings.append(
            ...
            "found in stages/106_inprocess.cpp. Refusing to state a scope "
    ...
    if max_len > max_read_len:
        ...
        findings.append(
            ...
            f"(MAX_READ_LEN, stages/106_inprocess.cpp). {n_over} distinct "
...
def main(argv):
    ...
    if not as_json:
        ...
        print(f"    MAX_READ_LEN = {limits['MAX_READ_LEN']}  (stages/106_inprocess.cpp)")
