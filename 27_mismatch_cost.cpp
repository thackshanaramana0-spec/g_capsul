        ...
        auto scan=[&](const std::string& text){
                ...
                th.emplace_back([&,t,lo,hi]{
                    ...
                    for(size_t p=from;p<hi;++p){
                        ...
                        for(uint32_t q=ix;q<mkey.size()&&mkey[q]==k;++q){
                            ...
                            if(mm<=MAXMAP){ hit[t].push_back({rid,(uint32_t)st}); hmm[t].push_back(mm); }
                        }
                    }
                });
            }
            ...
            for(size_t t=0;t<hit.size();++t)
                for(size_t q=0;q<hit[t].size();++q){
                    const auto& pr=hit[t][q];
                    if(!matched[pr.first]){ matched[pr.first]=1; ppos[pr.first]=pr.second;
                                            ++n_matched; mm_total+=hmm[t][q]; ++mm_reads;
                                            if(hmm[t][q]<256) ++mm_hist[hmm[t][q]]; }
                }
        };
        scan(pg);
