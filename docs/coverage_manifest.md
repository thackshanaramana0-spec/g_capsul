...
| `FAILURES_AND_REFUTED_IDEAS.md` | SUPERSEDED | Source for `CLAUDE.md` §6.3's 4 bugs, already cited throughout `claim1/*.md` |
...
| `PHASE2B_RESULT.md` | SUPERSEDED | Read in full; VOID-for-4-datasets note already in `CLAUDE.md` and cross-checked |
...
| `SECOND_REGION_SELF_MATCH.md` | SYNTHESIZED | `extras/SOTA_COMPARISON.md` (kept as-is); also in `CLAUDE.md`'s own "what is open" section |
...
- **`docs/working_notes/` (7 files)** — `ARCHIVE_CALLER_SPEED_RAM.md`,
  ...
  HERE INSTEAD" banner pointing to `README.md`/`CLAUDE.md`/`RESULT_CODE.md`/
  ...
  reflected in `CLAUDE.md`'s refuted-ideas table ("splitting positions by
...
| `CLAUDE.md` | Read extensively throughout — the project's own ground-truth reference, source for most of `refer_paper_docs/` |
