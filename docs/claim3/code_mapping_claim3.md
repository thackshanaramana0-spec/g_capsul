...
Purpose: Exact function/line references in src/decoder.cpp for
...
Keywords: code mapping, decoder.cpp, line numbers, function trace,
  contig_spans, find_occurrences, completion index, benchmark results, file index
---

# Code-to-table mapping

All line numbers are from `src/decoder.cpp` as committed at
...
- **Per-contig export via `contig_spans`**: `decoder.cpp:903-957`.
...
- **`contig_spans` written at encode time**: `src/encoder.cpp`,
...
- **Fast-exit before pg rebuild**: `decoder.cpp:417-490`
...
- **Sidecar load** (`.qidx` file): `decoder.cpp:518-586` approx —
...
- **T3.4's `rr` population** (pg-search): `find_occurrences()`, defined at
  `decoder.cpp` ~line 227 onward (pigeonhole seed-and-extend,
...
- **the completion index build (encode-adjacent, at `index` time)**: gated by `CAPS_XMI`
  and requiring `CAPS_PILEUP=1` first (see `decoder.cpp` around the
  `index` command's pileup/variant-site section, same function that writes
  `.sites`).

## T3.5 — Locus Retrieval

- **Not in `decoder.cpp` at all.** Built at the orchestration layer:
...
- Tally built during `index`, `decoder.cpp` ~line 1474-1512
  ...
  message and `BTR_NOTES.md`'s 2026-09-19 section for why it was not adopted).
...
| Number / claim | File |
|---|---|
| T3.1/T3.2/T3.3 speed table (all rows in files 01-03) | `results/claim3/claim3_T3.1_T3.2_T3.3.csv` |
...
| Session-level findings summary, dated | `BTR_NOTES.md`, "2026-09-19 session" section |
...
CSV (`results/claim3/claim3_T3.1_T3.2_T3.3.csv`) and the original
