...
int main(int argc,char** argv){
    ...
    // probe phase) -- physically compacts read `b` out of its own bucket so
    // future levels' scans no longer pay for it. O(current bucket length),
    // same cost class SPRING's remove() pays, and it shrinks over time.
    //
    // REMOVE_THRESH: measured (not guessed) that unconditional removal is a
    // net LOSS on well-behaved genomes -- C. elegans went 42.56s -> 45.97s
    // (+8%) because removal's O(bucket) shift cost is paid on every single
    // link, and normal genomes' buckets are already tiny (E. coli avg scan
    // length 0.12, most buckets size 1) -- there's nothing expensive being
    // saved, only pure overhead added. Skip removal below this size so the
    // common (small-bucket) case pays zero extra cost, matching baseline;
    // only pay the shift when a bucket is large enough that NOT shrinking it
    // would cost more across the sweep's remaining ~70-100 levels. CCAP*8=64
    // reuses the same real cutoff already established for MAXSCAN earlier
    // this session, not a new arbitrary number.
    const uint32_t REMOVE_THRESH=64;      // = CCAP(8)*8, CCAP is defined inside sweep() below
    auto removeRead=[&](uint32_t b){
        ...
        if(liveLen[s]<REMOVE_THRESH) return;  // small bucket: not worth the shift cost
        const uint32_t end=s+liveLen[s];
        for(uint32_t q=s;q<end;++q){
            if((uint32_t)(pent[q]&0xFFFFFFFFULL)!=b) continue;
            for(uint32_t t=q;t+1<end;++t) pent[t]=pent[t+1];
            --liveLen[s];
            return;
        }
    };
