...
int main(int argc,char** argv){
    ...
    std::vector<uint32_t> tails; tails.reserve(n);
    for(uint32_t i=0;i<n;++i) if(reads[i].size()>=MINOV) tails.push_back(i);
    ...
    fprintf(stderr,"seed probes=%zu links=%zu\n",probes,links);
    lap("descending-length sweep");
