...
int main(int argc,char**argv){
    ...
    auto blend=[&](uint32_t pos,int refc,uint32_t out[3]){
        ...
        // STAGE 59: Witten-Bell interpolation instead of a flat global W.
        // lambda(position) = C / (C + N) where C = total observations at
        // this position, N = number of DISTINCT symbols seen at it -- the
        // established n-gram-LM formula (decades of use in speech
        // recognition), chosen specifically because it gives high confidence
        // to CONSISTENT positions and low confidence to NOISY ones
        // automatically, per-position, with no hand-tuned global constant.
        // Matches what stage 57's analysis already found: 83.5% of
        // clustered sites are single-alt-base only (N=1, lambda climbs
        // fast); the rest have real disagreement (N=2-3, lambda stays low,
        // correctly distrusting a position that isn't actually consistent).
        uint64_t t[3];
        if(pf){
            uint64_t C=(uint64_t)(*pf)[0]+(*pf)[1]+(*pf)[2];
            int N=0; for(int k=0;k<3;++k) if((*pf)[k]) ++N;
            // lambda scaled into the same fixed-point range W already
            // sweeps (so W remains the one tunable "how strong can the
            // per-position signal ever get" ceiling, same sweep code below).
            const uint64_t lambda_num=C, lambda_den=C+(uint64_t)N;
            for(int k=0;k<3;++k)
                t[k]=(uint64_t)global[refc][k]
                    +((uint64_t)(*pf)[k]*W*lambda_num)/(lambda_den?lambda_den:1);
        } else {
            for(int k=0;k<3;++k) t[k]=(uint64_t)global[refc][k];
        }
