    ...
    while(*id_ptr){
        ...
        if(isalpha((unsigned char)*id_ptr)){
            ...
            if(prev_is_digit && match_len==token_len && !isdigit((unsigned char)prevc(prev_tok_ptr[token_ctr]+token_len))){
                ...
                // KEEP SPRING's own narrow path for its sweet spot -- a
                // dedicated single-byte model specialized to exactly this
                // range converges faster and codes cheaper than a wider
                // model would for the common small-positive case. Fixed
                // after stage 63's first attempt measured this: replacing
                // the narrow path outright made things WORSE (2.089 vs
                // 2.026 B/name), not better -- diluting the common case
                // into a coarser model cost more than the wider net saved.
                m.token_type[token_ctr].enc(enc,ID_DELTA);
                m.delta[token_ctr].enc(enc,(uint32_t)delta);
            } else if(prev_is_digit && token_ctr==24 && delta>=-32768 && delta<=32767){
                // Restricted to token index 24 ONLY (the Y tile coordinate
                // in this exact header format, confirmed by tokenizing a
                // real header by hand) after testing the widened path on
                // ALL digit tokens made things worse (2.055 vs 2.026): X's
                // delta has stdev ~18,710 (near the full coordinate range),
                // so applying this to X fires on essentially-random deltas
                // that happen to land in range by chance, diluting the
                // model with noise the narrow SPRING path never had to pay
                // for. This is real X/Y asymmetry, not guessed.
