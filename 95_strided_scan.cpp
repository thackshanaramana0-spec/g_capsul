...
int main(int argc,char** argv){
    ...
    // STAGE 95 -- real fix for stage 93/94's finding: `pent` is sorted by
    // (seed<<32 | read_id), so a repeat bucket's entries are in ASCENDING
    // READ-ID order, not random. Sequential truncation at MAXSCAN therefore
    // always samples the SAME low-read-id entries every time, systematically
    // biased -- not a fair sample of the bucket. Confirmed this, not assumed:
    // stage 94 showed T. cacao's tail is real signal (13.5-60.5% real link
    // loss under truncation), not noise a cap should discard.
    //
    // Fix: find the bucket's true end via upper_bound (binary search on the
    // already-sorted pent array, O(log N), cheap -- no extra scan needed to
    // discover bucket length). When a bucket exceeds MAXSCAN, STRIDE evenly
    // across the whole bucket instead of truncating to the front, so the same
    // bounded work budget samples representatively across read-id space
    // rather than always the same low-id subset. This is the standard
    // technique for bounded-cost repeat handling (same idea behind minimizer/
    // reservoir sampling in real genome indices) -- not a novel guess.
    auto pbucketEnd=[&](uint32_t pix)->uint32_t{
        const uint32_t k=(uint32_t)(pent[pix]>>32);
        if(k==UINT32_MAX) return (uint32_t)pent.size();  // max key: bucket runs to array end
        // first entry whose top-32 bits exceed k: every entry with top32==k
        // is < (k+1)<<32 (its low 32 bits, a read id, is < 2^32), and every
        // entry with a higher key is >= (k+1)<<32 -- exact boundary.
        const uint64_t hikey=((uint64_t)(k+1))<<32;
        auto it=std::lower_bound(pent.begin()+pix,pent.end(),hikey);
        return (uint32_t)(it-pent.begin());
    };
    buildPref(false);
    ...
    for(uint32_t L=Lmax-1; L>=sweep_minov && L>=SW; --L){
        ...
        ccnt.assign(w,0);
        {
            ...
            for(long long ii=0;ii<(long long)w;++ii){
                ...
                uint8_t c=0;
                uint32_t scanlen=0;
                const uint32_t bend=pbucketEnd(pix);
                const uint32_t blen=bend-pix;
                if(blen<=MAXSCAN){
                    // small bucket: exact sequential scan, unchanged behaviour
                    ...
                    // STAGE 95: large bucket (repeat-driven seed collision) --
                    // stride evenly across [pix,bend) instead of truncating
                    // to the front, so the bounded MAXSCAN budget samples
                    // representatively across the whole bucket's read-id
                    // range rather than always the same low-id subset.
                    const double stride=(double)blen/(double)MAXSCAN;
                    for(uint32_t s=0;s<MAXSCAN;++s){
                        const uint32_t q=pix+(uint32_t)((double)s*stride);
                        ++scanlen;
                        const uint32_t b=(uint32_t)(pent[q]&0xFFFFFFFFULL);
                        if(b==a) continue;
                        if(!admit[b]) continue;
