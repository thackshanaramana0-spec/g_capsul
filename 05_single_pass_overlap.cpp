...
int main(int argc,char** argv){
    ...
    size_t tried=0,verified=0;
    for(uint32_t a=0;a<n;++a){
        const std::string& A=reads[a];
        if(A.size()<MINOV) continue;
        const uint32_t maxoff=(uint32_t)A.size()-MINOV;      // smallest overlap allowed
        ...
        for(uint32_t off=1;off<=maxoff;++off){
            ...
            if(!have){ if(!pack(A.data()+off,k)){ have=false; continue; } have=true; }
                ...
                k=((k<<2)|(uint64_t)v)&SEEDMSK;              // roll one base
            }
            ++tried;
            auto it=pref.find(k); if(it==pref.end()) continue;
            const uint32_t ov=(uint32_t)A.size()-off;        // implied overlap length
            for(uint32_t b:it->second){
                if(b==a) continue;
                const std::string& B=reads[b];
                if(B.size()<ov) continue;
                if(memcmp(A.data()+off,B.data(),ov)!=0) continue;
                best[a]={b,ov}; ++verified; break;           // first hit == longest
            }
            if(best[a].rid!=NONE) break;                     // outward walk stops here
        }
    }
    fprintf(stderr,"seed probes=%zu  verified=%zu\n",tried,verified);
    lap("longest-overlap search");

    // ── greedy: commit links in globally descending overlap order ────────────
    ...
    for(uint32_t a:order){
        const uint32_t b=best[a].rid;
        if(nxt[a]!=NONE||prv[b]!=NONE||ch_h[a]==b) continue;   // O(1) cycle refusal
