...
int main(int argc,char**argv){
    ...
    std::vector<uint8_t> pos=loadb(argv[3]), ctx3=loadb(argv[4]);
    ...
    // STAGE 60: stack stage 53's context (ref x read-pos-bucket x prev-base,
    // 48 contexts) as the GLOBAL/fallback model instead of stage 58's plain
    // 4-context ref-only global -- so a never-seen position still gets the
    // benefit of stage 53's real (if smaller) signal, and a repeated
    // position gets BOTH effects at once, not one replacing the other.
    std::array<uint32_t,3> global[48]; for(auto& f:global) f={1,1,1};
    auto posBucket=[&](uint8_t p)->int{ return p<50?0:p<100?1:2; };
    auto gctx=[&](int refc,int pb,int pv)->int{ return (refc*3+pb)*4+pv; };
    ...
    auto blend=[&](uint32_t pos,int gx,uint32_t out[3]){
        ...
        uint64_t t[3];
        for(int k=0;k<3;++k) t[k]=(uint64_t)global[gx][k]+(pf?(uint64_t)(*pf)[k]*W:0ULL);
    ...
    for(size_t i=0;i<n;++i){
        ...
        const int gx=gctx(r,pb,pv);
        uint32_t f[3]; blend(pgpos[i],gx,f);
        uint32_t tot=f[0]+f[1]+f[2];
        uint32_t lo=0; for(int j=0;j<k;++j) lo+=f[j];
        enc.encode(lo,lo+f[k],tot);
        // update BOTH models -- global (now stage53-shaped) always, per-position lazily created
        global[gx][k]+=8; { uint32_t t2=global[gx][0]+global[gx][1]+global[gx][2];
            if(t2>65536) for(int j=0;j<3;++j) global[gx][j]=(global[gx][j]>>1)|1; }
