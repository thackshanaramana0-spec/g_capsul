#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════
#  SANITY — ONE dataset, ALL THREE CLAIMS, end to end.
#
...
/usr/bin/time -v env CAPS_NAMES=1 CAPS_QUAL=1 INPUT="$SRC" ARCHIVE="$A" BEST="$BEST" \
...
if [ -s "$DATA_DIR/${DS}_pooled.fq" ] && [ -s "$REFS/chr20.fa" ]; then
  step "our caller (SNV configuration, one pass with compression)"
  C2="$OUT/claim2"; mkdir -p "$C2"
  CAPS_DBG=1 CAPS_DBG_ONLY=1 \
    bash "$HERE/scripts/run_fullchr20_bench_capsule.sh" "$BEST" "$HERE/scripts" \
         "$REFS/chr20.fa" "$SRC" "$DS" "$C2" > "$OUT/claim2.log" 2>&1
