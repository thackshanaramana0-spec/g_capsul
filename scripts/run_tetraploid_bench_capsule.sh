...
if [ "${CAPS_ENCODER_PATH:-0}" = 1 ]; then
    ...
    export CAPS_PLOIDY="$PLOIDY" CAPS_DUMP_CONTIGS="$OUT/contigs.tsv"
    env CAPS_CALL_INDELS=1 CAPS_PLOIDY="$PLOIDY" "$DEC" call "$OUT/reads.capsule" "$OUT/calls.vcf" "$OUT/callwk" \
