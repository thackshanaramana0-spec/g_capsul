...
#
#   usage: run_fullchr20_bench_capsule.sh <capsule_exe> <scripts_dir> <ref.fa> \
#                                         <reads.fq> [individual] [outdir]
set -e
T_START=$(date +%s)
...
[ -s "$READS" ] || { echo "no reads file: $READS" >&2; exit 1; }
...
log "[1/5] running CAPSULE caller (CAPS_CALL=1) on $NR_ reads — this is the long step"
export CAPS_CALL=1 CALL_VCF="$OUT/calls.vcf" CAPS_DUMP_CONTIGS="$OUT/contigs.tsv"
if [ ! -s contigs.tsv ]; then
  /usr/bin/time -v "$CAPS" "$READS" 3 16 16 22 16 16 1 24 64 1 \
